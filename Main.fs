...
let rec filter_deck (D : deck) : deck =
    match D with 
        [] -> []
        | x::xs -> 
            if ((x.typee <> "MINION") || (x.attack <= 0) || (x.health <= 0)) then filter_deck xs
...
(*Inizio prima implementazione - iterativa*)
//funzione get_card_list che dato un giocatore e il costo in mana mi restituisce una lista delle carte possibilmente giocabili
let get_card_list (mana : int) (P : player) : card list =
    ...
    in aux P.deck 0.0 [] 
(*Fine prima implementzione*)

(*Inizio seconda implementazione - ricorsiva*)
//funzione get_card_list che dato un giocatore e il costo in mana mi restituisce una lista delle carte possibilmente giocabili
let punteggio (x : card) : float = float (x.attack) / float (x.health)

//restituisce una carta col punteggio massimo
let rec punteggio_massimo (D : deck) (mana : int) : card =
    match D with 
        [] -> carta_nulla
        | x::xs -> if (punteggio x > punteggio (punteggio_massimo xs mana) && x.cost <= mana) then x
                   else punteggio_massimo xs mana 

//crea una lista di tutte le carte giocabili col punteggio massimo uguale
let rec get_card_list_2 (mana : int) (P : player) : card list =
    let deck_p : deck = P.deck
    let c_max : card = punteggio_massimo deck_p mana in
    let rec aux (D : deck) =
        match D with 
            [] -> []
            | x::xs -> if (punteggio x = punteggio c_max && x.cost <= mana) then x::(aux xs)
                       else aux xs
    in aux deck_p
(*Fine seconda implementazione*)

//funzione che seleziona in maniera casuale una delle carte giocabili
let rec draw_card (mana : int) (P : player) : card =
    let c = get_card_list mana P in               //se qui si volesse utilizzare la funzione iterativa piuttosto che la ricorsiva, sostituire "get_card_list_2" con "get_card_list"
    let n : int = rand.Next (0 , c.Length) in
    //n è un numero random tra 0 incluso e la lunghezza della lista di carte giocabili non inclusa. Giochiamo l'ennesimo elemento della lista:
...
let fight (deck1 : deck) (deck2 : deck) : player * player * int =
   ...
   //inizializziamo il contatore di turno
   let mutable turn = 1
   //inizializziamo un valore booleano per uscire dal ciclo in qualsiasi momento
   let mutable quit = false
   //stampiamo le condizioni iniziali dei 2 giocatori
   printfn "\nINITIAL CONDITIONS : \nplayer1 = %s\nplayer2 = %s \n" (pretty_player p1) (pretty_player p2)
   //while: ogni iterazione rappresenta un turno di gioco
